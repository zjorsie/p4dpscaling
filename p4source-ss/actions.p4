...
action apply_packetCounter_stateful() {
    ...
    // turn off for debug purposes
    modify_field(_ss_m.pktnum, pktStats_metadata.pktstats);
...
action apply_packetCounter_stateful_flowID() {
    ...
    modify_field(_ss_m.pktnum, packetCounter_metadata.packetNum);
    modify_field(_ss_m.pktnum, pktStats_metadata.pktstats);
}
...
action forward_ss() {
    // forward packet out port 2 (only for switch s4)
    modify_field(standard_metadata.egress_spec, 0x02);
}
action forward_ss_append() {
    // forward packet out port 2 (only for switch s4)
    add_header(_ss_id);
    // leave data empty and forward whole packet:
    modify_field(_ss_id.hdr, MIG_SS);
    modify_field(_ss_id.data, _ss_m.pktnum);
    modify_field(standard_metadata.egress_spec, 0x02);
}

action clone_ss() {
    // send a clone to s2
    modify_field(local_metadata.mirror_session, 0x32);
    clone_i2e(0x32, mirror_fld_list);
    modify_field(_ss_m.state, _ss_m.state);
}

action read_ss_state() {
    register_read(_ss_m.state, _ss_flowState, routing_metadata.flowID_hash);
}

action set_new_state() {
    register_write(_ss_flowState, routing_metadata.flowID_hash, _mig_flow_recv.subProtocol);
}

action send_ss_to_cpu() {
    modify_field(standard_metadata.egress_spec, CPU_PORT);
}

action _ss_putdown() {
    register_write(_ss_flowState, routing_metadata.flowID_hash, 0x0f);
    register_write(reg_packetCounter, routing_metadata.flowID_hash, _ss_id.data);
    modify_field(_ss_id.data, 0x00000f0f);
}

action ss_append_header() {
    // read register for correct state value:
    modify_field_with_hash_based_offset(routing_metadata.flowID_hash, 0, migration_tbl_flowID_calculation, HASH_SIZE);
    add_header(_ss_id);
    // leave data empty and forward whole packet:
    modify_field(_ss_id.hdr, MIG_SS);
    modify_field(_ss_id.data, _ss_m.pktnum);    
    register_read(_ss_id.data, reg_packetCounter, routing_metadata.flowID_hash);
    // leave data empty and forward whole packet:

}

action send_ss_mirror_to_cpu() {
    modify_field(standard_metadata.egress_spec, CPU_PORT);
    modify_field(_ss_id.data, 0x0000f0f0);
}