...
class P4InfoHelper(object):
    ...
    def get_all_id(self,entity_type):
        # get all instances of certain entity type
        idList = []
        for o in getattr(self.p4info, entity_type): 
            pre = o.preamble
            idList.append(pre.id)
        return idList
    def get_type_info(self):
        # This function returns the type_info as a
        return google.protobuf.json_format.MessageToDict(self.p4info.type_info)

    def get_digest_fields_by_id(self,digest_id):
        # this function retrieves the fields used by a digest message. First, it tries out the direct way.
        # If the member is a struct itself, it resolves this and then tries to go to the 'bitstring'
        for d_id in self.get_all_id('digests'):
            if digest_id == d_id:
                field_list = self.get('digests',id=digest_id).type_spec.struct.name
                type_infoDict = json.loads(google.protobuf.json_format.MessageToJson(self.p4info.type_info))
                outList = []
                counter = 0
                while counter < 5:
                    counter += 1
                    for member in type_infoDict['structs'][field_list]['members']:
                        if 'struct' in member['typeSpec']:
                            field_list = member['typeSpec']['struct']['name']
                        if 'bitstring' in member['typeSpec']:
                            outList.append({member['name']: member['typeSpec']['bitstring']['bit']['bitwidth']}) 
                            #outDict[member['name']] = member['typeSpec']['bitstring']['bit']['bitwidth']
                    if len(outList) > 0:
                        # elements have been found, exit:
                        return outList
        return None
